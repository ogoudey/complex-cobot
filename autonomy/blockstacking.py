...
class BlockStackingProblem():
	def __init__(self, domain=None):
		...
		"""
		drop = unified_planning.model.InstantaneousAction('drop', b=Block, l_over=Location, l_under=Location) # you still drop at l_over
		b = drop.parameter('b')
		l_over = drop.parameter('l_over')
		l_under = drop.parameter('l_under')
		drop.add_precondition(grasped(b))
		drop.add_precondition(g_at(l_over))
		drop.add_precondition(over(l_over, l_under))
		drop.add_precondition(b_at(b, l_over)) # may be redundant
		drop.add_effect(air(l_over), False)
		drop.add_effect(grasped(b), False)
		drop.add_effect(ungrasped(), True)
		drop.add_effect(hoverable(l_over), True)
		drop.add_effect(hoverable(l_under), False)
		"""
