...
class MinimalCarProblem(MinimalProblem):
    def __init__(self):
        super().__init__()
        #self.inventory = tracker.InventoryTracker() # Additional planning thing
...
class CarProblem(Problem):
    def __init__(self, mp, state):
        ...
        """
        self.problem.set_initial_value(c_at(self.problem.object(state.location)), True)
        
        self.problem.set_initial_value(air(self.problem.object(state.location)), False)
        
        self.problem.set_initial_value(t_at(self.problem.object(state.thing_location), thing), True)
...
class MinimalBlockStackingProblem(MinimalProblem):
